...
#[cfg(feature = "kzg-rs")]
pub mod c_kzg_precompile {
    use crate::{Address, Error, PrecompileResult};
    use c_kzg::{Bytes32, Bytes48, KzgProof, KzgSettings};
    use once_cell::race::OnceBox;
    use revm_primitives::kzg::{G1_POINTS, G2_POINTS};
    use revm_primitives::{hex_literal::hex, Bytes, PrecompileOutput};
    use sha2::{Digest, Sha256};
    ...
    pub fn run(input: &Bytes, gas_limit: u64) -> PrecompileResult {
        ...
        DEFAULT.get_or_init(|| {
            let settings = KzgSettings::load_trusted_setup(G1_POINTS.as_ref(), G2_POINTS.as_ref())
        ...
        let y = as_bytes32(&input[64..96]);
        let proof = as_bytes48(&input[144..192]);
        if !verify_kzg_proof(commitment, z, y, proof, DEFAULT.get().unwrap()) {
            return Err(Error::BlobVerifyKzgProofFailed.into());
        }
